...
export default function RushingD() {
  ...
  const getRepositories= () => {
    fetch('https://nfl-team-stats.p.rapidapi.com/v1/nfl-stats/teams/rushing-stats/offense/2022', options)
...
useEffect(() => {
  ...
  return (
    ...
    Offensive rushing stats from this year! </Box>
      <FlatList
          data={repositories}
          ...
          renderItem={({ item }) => (
            <Box width="100%" bg="primary.50" p="4" shadow={2} _text={{
              fontSize: "xs",
              fontWeight: "bold",
              color: "black" }}>{item.name}{item.yards}yards </Box>
          )}
